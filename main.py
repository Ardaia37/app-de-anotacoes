...
# função do botao (adicionar tarefas)
def adicionar_tarefas():
    ...
    if tarefa and tarefa != "Escreva sua tarefa aqui":
        ...
        messagebox.showwarning("Entrada Invalida","por favor, ensira uma tarefa", )
def atualizar_tarefa(entrada):
...
def adicionar_item_tarefa(tarefa):
    ...
    botao_editar = tk.Button(frame_tarefa, image=icon_edit, command=lambda f=frame_tarefa, l=label_tarefa: preparar_edicao(f, l), bg="white", relief=tk.FLAT)
    ...
    checkebutton = ttk.Checkbutton(frame_tarefa, command=lambda label=label_tarefa: alterar_sublinhado(label))
    checkebutton.pack(side=tk.RIGHT, padx=5)
...
icon_deletar = PhotoImage(file="./fotos_png/delete.png").subsample(3, 3) 
icon_edit = PhotoImage(file="./fotos_png/edit.png").subsample(3, 3) 

# criando o titulo do APK
fonte_cabecalho = font.Font(family="Garamond", size=24, weight="bold")
rotulo_cabecalho = tk.Label(janela, text="App de Anotações", font=fonte_cabecalho, bg="#F0F0F0", fg="#333").pack(pady=20)
...
botao_adicionar = tk.Button(frame, command=adicionar_tarefas, text="Adicionar Tarefa", bg="#4CAF50", fg="white", height=1, width=15, font=("robote", 11), relief=tk.FLAT)
